...
- bundled license builder CLI
- launcher wrappers for console and GUI entry points
- per-user writable runtime materialized under `~/.local/state/stata18-runtime/runtime`
- per-user config under `~/.config/stata18-runtime/config.env`

## GitHub Release Automation

A repository-facing release path now exists alongside the local packaging work. The new workflow at `.github/workflows/release.yml` runs on tags or manual dispatch, downloads the upstream Stata 18 media from Duke, verifies the known SHA-256, stages GTK2 and ncurses5 compatibility libraries from an Arch environment, builds the package with `makepkg`, and publishes the resulting release assets through GitHub Releases.

The PKGBUILD now accepts `GTK2_LIB_ROOT` and `NCURSES5_LIB_ROOT` overrides so CI does not depend on this workstation's local staging paths.
